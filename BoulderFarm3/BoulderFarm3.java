...
public class BoulderFarm3 {
    ...
    public BoulderFarm3() throws AWTException {
        foulTarnished = new Robot();
        String version = "Boulder Farm 3.0";
        LocalDateTime time;
        int startSecond, startMinute, startHour, endSecond, endMinute, endHour;
        final double LAP_TIME = 37.34;
        ...
        foulTarnished.mouseMove(1280, 720);

        // Initial Menu
        String[] menuStrings = {
                "                   Welcome to BoulderFarm!\n",
                "\nEach lap takes 37.34 seconds and earns 1952 runes        ",
                "\n    Laps earn 2342 runes with the Golden Scarab",
                "\n           How many laps would you like to run?"
        };

        int iterations = Integer.parseInt(JOptionPane.showInputDialog(null, menuStrings[0] + menuStrings[1] +
                menuStrings[2] + menuStrings[3], version, JOptionPane.QUESTION_MESSAGE));
        ...
        JOptionPane.showMessageDialog(null, startStrings[0] +
                startStrings[1] + startStrings[2], version, JOptionPane.INFORMATION_MESSAGE);

        // Start time calculation
        time = LocalDateTime.now();
        startSecond = time.getSecond();
        ...
        time = LocalDateTime.now();
        endSecond = time.getSecond();
        endMinute = time.getMinute();
        endHour = time.getHour();

        // Rune calculation
        runes = BOULDER * iterations;
        scarabRunes = SCARAB_BOULDER * iterations;
        ...
        JOptionPane.showMessageDialog(null, endingStrings[0] + endingStrings[1] + endingStrings[2] +
                endingStrings[3] + endingStrings[4] + endingStrings[5], version, JOptionPane.INFORMATION_MESSAGE);
    }

    // The commands to be executed for each lap
